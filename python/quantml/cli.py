...
from .rag.news import load_ticker_news
...
def main() -> None:
    ...
    if args.real_data:
        # The sample corpus only covers the fictional demo ticker ACME, so
        # for a real ticker the sentiment signal comes from real Yahoo
        # Finance headlines instead (recent stories only -- on a
        # multi-year backtest the sentiment tilt covers just the recent
        # tail, since that's all the news that's actually available).
        news_docs = load_ticker_news(args.ticker)
        docs = docs + news_docs
        if news_docs:
            print(f"Loaded {len(news_docs)} real news headlines for {args.ticker}\n")
    ...
    print(f"Retrieved {len(docs)} docs, {sentiment.shape[0]} scored days for {args.ticker}\n")
